...
import { ProductParams } from '../interfaces/products.interface';
 ...
 `;
  ...
  getMerchantProductById(id: number): Observable<any> {
    return this._http.get(
      `${environment.URL_API}MerchantProducts/definitions/${id}`
    );
  }

  searchForMerchantProduct(keyword: any): Observable<any> {
    return this._http.get(
      `${environment.URL_API}MerchantProducts/true/${keyword}`
    );
  }
  ...
  rejectProduct(data: {
    ...
    return this._http.put(
      `${environment.URL_API}MerchantProducts/change-status`,
      data
    );
  }
  approveProduct(data: {
